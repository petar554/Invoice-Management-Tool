...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowedTypes = (
      process.env.ALLOWED_FILE_TYPES || "pdf,docx,xml"
    ).split(",");
...
// DEVELOPMENT CODE (if you need to test without auth, uncomment below and comment above)
/*
if (process.env.NODE_ENV === "development") {
  ...
  router.use((req, res, next) => {
    if (!req.user) {
      req.user = {
        id: "d9740922-c74c-4444-9898-f89acef09000",
    ...
    try {
      ...
      if (search) {
        const { data: searchData, error: searchError } =
          await supabaseAdmin.rpc("search_documents", {
            search_query: search,
            user_id: userId,
          });
...
router.post("/upload", upload.array("documents", 5), async (req, res, next) => {
  ...
  // Auth check (middleware should have already set req.user)
  if (!req.user) {
  ...
  console.log("Request details:", {
    ...
    body: req.body,
    fileFieldName: req.fileFieldName || "not set",
  });

  try {
    // Check if multer middleware processed files
    if (!req.files || req.files.length === 0) {
      console.log("No files uploaded - checking field names");
      console.log(
        "Expected field: 'documents', received fields:",
      ...
      return res.status(400).json({
        ...
        message: "Please ensure files are sent with field name 'documents'",
      });
    }
    ...
    for (const file of req.files) {
      try {
        ...
        console.log("Attempting to insert document with data:", {
          filename,
        ...
        console.log("Successfully inserted document:", documentData);
